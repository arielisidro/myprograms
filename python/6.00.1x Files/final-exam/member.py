...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        def getDistance(node):
            d=0
            n=node
            while n.get_parent() != None:
                d+=1
                n=n.get_parent()
            return d   

        def getRemoved(distanceA,distanceB):
            if( distanceA > distanceB):
                return distanceA-distanceB
            else:            
                return distanceB-distanceA
            
        def getCousinType(nodeA,nodeB,distanceA,distanceB):
            ...
            if( distanceA > distanceB+1):
                for i in range(distanceA-distanceB-1):
                    parentA = parentA.get_parent()
            elif(distanceB > distanceA+1): 
                for i in range(distanceB-distanceA-1):
            ...
            if(distanceA > distanceB):
                while not parentA.is_parent(ParentB):
                    parentA = parentA.get_parent()
                    ParentB = ParentB.get_parent()
                    cousinType += 1
            elif distanceB>distanceA:
                while not ParentB.is_parent(parentA):
                    parentA = parentA.get_parent()
                    ParentB = ParentB.get_parent()
                    cousinType += 1
        ...
        if nodeA.is_parent(nodeB) or nodeB.is_parent(nodeA) or nodeA==nodeB:
            return (-1,0)
        ...
        removed=getRemoved(distanceA,distanceB)

        cousinType=getCousinType(nodeA,nodeB,distanceA,distanceB)
