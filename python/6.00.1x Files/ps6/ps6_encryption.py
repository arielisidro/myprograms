...
def getStoryString():
    ...
    """
    return open(os.path.dirname(__file__)+"/story.txt", "r").read()
...
def findBestShift(wordList, text):
    ...
    shift=0;
    for s in range(26):
        textSplit=applyShift(text.lower(),s).split(' ')
        numberValidWords=0
        for w in textSplit:
            if isWord(wordList,w):
                numberValidWords+=1
                
        if numberValidWords>maxValidWords:
            maxValidWords=numberValidWords
            shift=s
